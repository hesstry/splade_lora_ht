...
def bm25_score(freq, norm_len, fdf, Ntotal = 8841823, k1 = 0.9, b = 0.4):
    ftf = float(freq)
    tf_weight = ftf / (ftf + k1 * (1.0 - b + b * norm_len/55))
    idf_weight = max(1.0E-6, math.log((Ntotal * 1.0 - fdf + 0.5) / (fdf + 0.5)))
    bm25 = int(50*(tf_weight * idf_weight * (1+k1)))
    # print(bm25)
    return bm25
...
class SentenceTransformerA(SentenceTransformer):
    ...
    def fit(self,
        ...
        fdf_json = json.load(open('fdf.json'))
        fdf_dict = {}
        for x in fdf_json:
            fdf_dict[int(x)] = fdf_json[x]
        ...
        for epoch in trange(epochs, desc="Epoch", disable=not show_progress_bar):
            ...
            for train_idx in range(num_train_objectives):
                ...
                bm25_pos = torch.zeros(train_batch_size, 30522, device=self._target_device)
                bm25_neg = torch.zeros(train_batch_size, 30522, device=self._target_device)

                for i in range(train_batch_size):
                    q = list(features[0]['input_ids'][i])
                    filter(q)
                    dpos = list(features[1]['input_ids'][i])
                    filter(dpos)
                    dneg = list(features[2]['input_ids'][i])
                    filter(dneg)

                    for x in q:
                        if x in fdf_dict:
                            bm25_pos[i][x] += 50 * bm25_score(dpos.count(x), len(dpos), fdf_dict[x])
                            bm25_neg[i][x] += 50 * bm25_score(dneg.count(x), len(dneg), fdf_dict[x])

                # bm25_pos.to('cuda')
                # bm25_neg.to('cuda')

                # print('out', bm25_pos, bm25_neg)
                # print(self._target_device, bm25_pos.device)

                if use_amp:
                    with autocast():
                        loss_value = loss_model(features, labels, bm25_pos, bm25_neg)
                    ...
                    loss_value = loss_model(features, labels, bm25_pos, bm25_neg)/accum_iter
                    loss_value.backward()
                    torch.nn.utils.clip_grad_norm_(loss_model.parameters(), max_grad_norm)
